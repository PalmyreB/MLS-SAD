...
public class CompleteJavaFileCreator implements ICodeLevelModelCreator {
	 ...
	 */
	public CompleteJavaFileCreator(final String aSourcePathEntry, final String aClasspathEntry) {

		// the folder of the source code to analyze well organized like a project
		// final String sourcePathEntry = "./rsc/src/";
		// how to throw an exception here
		if (!new File(aSourcePathEntry).exists()) {
			throw new RuntimeException(new FileNotFoundException("The source does not exist " + aSourcePathEntry));
		}
		...
		try {
			this.javaProject = new FileSystemJavaProject(Arrays.asList(classpathEntries),
	...
	public CompleteJavaFileCreator(final String aSourcePathEntry, final String aClasspathEntry,
		...
		this(new String[] { aSourcePathEntry }, new String[] { aClasspathEntry }, someSourceFiles);
	}

	public CompleteJavaFileCreator(final String[] someSourcePathEntries, final String[] someClasspathEntries,
			final String[] someSourceFiles) {

		for (final String path : someSourcePathEntries) {
			if (!new File(path).exists()) {
				throw new RuntimeException(new FileNotFoundException("The source does not exist " + path));
			}
		}

		try {
			this.javaProject = new FilesAndDirectoriesJavaProject(Arrays.asList(someClasspathEntries),
					Arrays.asList(someSourcePathEntries), Arrays.asList(someSourceFiles));
	...
	private void createModelFromSource(final ICodeLevelModel aCodeLevelModel, final SourceInputsHolder javaProject) {
