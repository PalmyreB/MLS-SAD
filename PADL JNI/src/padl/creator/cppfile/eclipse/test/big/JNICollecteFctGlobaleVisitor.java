...
import padl.cpp.kernel.ICPPParameter;
...
import padl.cpp.kernel.impl.CPPParameter;


public class JNICollecteFctGlobaleVisitor implements ICPPWalker {
	...
	private IGlobalFunction currentglobalfunction;
	...
	public void open(IGlobalFunction aGlobalFunction)
	 {
		...
		this.currentglobalfunction=aGlobalFunction;
		 listOfcurrentglobalfunction.add(aGlobalFunction.getDisplayName());
	 }
	...
	public void open(ICPPClass aCPPClass) {
		...
		this.currentcppclass=aCPPClass;
		 listOfcppclasss.add(aCPPClass.getDisplayName()
					);
	}
	...
	public void open(IMethod aMethod) {
		...
		// listOfcurrentmethod.add(aMethod.getDisplayName());
	...
	/*	if ((aParameter.getDisplayName().contains("env"))
				&&
				 (aParameter.getDisplayTypeName().contains("ProblemType"))
				)
		{ listOfJNIMethods.add(this.currentMethod.getDisplayName()
						);
	...
	@Override
	public void visit(ICPPParameter aCPPParameter) {
		// TODO Auto-generated method stub
		if ((aCPPParameter.getDisplayTypeName().contains("JNIEnv *"))
				// &&
				//(aCPPParameter.getDisplayName().contains("*"))
				)			
		{
			 //listOfJNIMethods.add(aCPPParameter.getDisplayTypeName());
			
		//	 System.out.println(aCPPParameter.getDisplayName());
			 listOfJNIMethods.add(this.currentglobalfunction.getDisplayName()
					);
