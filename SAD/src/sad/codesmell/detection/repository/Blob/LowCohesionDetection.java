	...
	public void detect(final IAbstractLevelModel anAbstractLevelModel) {
		...
		while (iter.hasNext()) {
			...
			if (entity instanceof IClass) {
				...
				final double LCOM5 = ((IUnaryMetric) MetricsRepository.getInstance().getMetric("LCOM5"))
						.compute(anAbstractLevelModel, aClass);
				mapOfLowCohesionValues.put(aClass, new Double[] { new Double(LCOM5), new Double(0) });
				// final double LCOM5 = ((IUnaryMetric)
				// MetricsRepository.getInstance().getMetric("LCOM5")).compute(anAbstractLevelModel,
				// aClass);
				// mapOfLowCohesionValues.put(aClass, new Double(LCOM5));
			}
		}
		...
		if (thereIsLowCohesion == true) {
			...
			final Iterator iter3 = mapOfLowCohesionClassesFromBoxPlot.keySet().iterator();
			...
			while (iter3.hasNext()) {
				...
				try {
					...
					final double LCOM5 = ((IUnaryMetric) MetricsRepository.getInstance().getMetric("LCOM5"))
							.compute(anAbstractLevelModel, aLowCohesionClass);
					...
					classProp.addProperty(new MetricProperty("LCOM5", LCOM5, thresholdMap, fuzziness.doubleValue()));
